...
**セル1（認証用）:**
- 本リポジトリの `auth.py` の**全内容**をコピーしてセルに貼り付け

**セル2（メイン処理）:**
- 本リポジトリの `main.py` の**全内容**をコピーしてセルに貼り付け

4. セル1を実行（初回は Google ログイン画面が出現）
5. セル2を実行して動作確認

**重要な注意事項:**
- ⚠️ セル2では `!python main.py` のようなコマンドは**使用しないでください**
- セル2には `main.py` の内容を**直接貼り付けて**実行してください
- セル間で変数（`gc` と `creds`）を共有するため、ファイルとして実行すると動作しません

**ヒント:**
- GitHubの各ファイルページで「Raw」ボタンをクリックすると、コピーしやすい形式で表示されます
- ノートブックを保存すれば、次回からはセルを実行するだけで使用可能
- セル1とセル2は順番に実行する必要があります（セル1 → セル2）
...
| 2.1.1 | 2025-11-30 | バグ修正：セル間での変数共有を修正、実行手順の明確化 |
